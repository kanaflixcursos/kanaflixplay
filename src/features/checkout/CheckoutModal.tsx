...
export function CheckoutModal({ open, onOpenChange, course, onSuccess }: CheckoutModalProps) {
  ...
  const fetchAddressByCep = async (cep: string) => {
    ...
    try {
      ...
      if (!data.erro) {
        setAddress(a => ({ ...a, street: data.logradouro || '', neighborhood: data.bairro || '', city: data.localidade || '', state: data.uf || '' }));
      }
  ...
  const handleSubmit = async () => {
    ...
    if (paymentMethod === 'credit_card' && (!card.number || !card.holderName || !card.expMonth || !card.expYear || !card.cvv)) {
  ...
  const paymentMethods = [
    { method: 'pix' as const, icon: QrCode, label: 'PIX', badge: null, description: 'Aprovação imediata' },
    ...
    <div className="flex flex-col h-full">
      {paymentResult ? (
        <div className="p-6 space-y-6">
          {paymentResult.pix && (
            <div className="text-center space-y-6">
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-success/10 text-success rounded-full text-sm font-medium">
                <Zap className="h-4 w-4" />PIX Gerado com Sucesso
              </div>
            ...
            <div className="space-y-6">
              <div className="text-center">
                <div className="inline-flex items-center gap-2 px-4 py-2 bg-success/10 text-success rounded-full text-sm font-medium mb-6">
                  <Check className="h-4 w-4" />Boleto Gerado
                </div>
                <Button asChild size="lg" className="w-full gap-2 h-12">
                  <a href={paymentResult.boleto.url} target="_blank" rel="noopener noreferrer"><ExternalLink className="h-4 w-4" />Visualizar Boleto</a>
                </Button>
              </div>
              <div className="space-y-3">
          ...
          <Button variant="outline" className="w-full" onClick={() => { resetModal(); onOpenChange(false); }}>Fechar</Button>
        </div>
      ) : (
        <div className="flex flex-col h-full">
          {/* Course Summary */}
          <div className="p-4 md:p-6 bg-gradient-to-br from-primary/5 via-transparent to-accent/5 border-b">
            <div className="flex gap-4">
              {course.thumbnail_url && (
                <img src={course.thumbnail_url} alt={course.title} className="w-20 h-14 object-cover rounded-lg border shrink-0" />
              )}
            ...
            <div className="mt-4 flex items-baseline justify-between">
              <span className="text-3xl font-bold text-foreground">{formatPriceBRL(course.price)}</span>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6">
              ...
              <div className="grid grid-cols-3 gap-2">
                {paymentMethods.map(({ method, icon: Icon, label, badge, description }) => (
                  <button key={method} type="button" onClick={() => setPaymentMethod(method)}
                    ...
                    <span className={`text-xs font-medium ${paymentMethod === method ? 'text-primary' : 'text-foreground'}`}>{label}</span>
            ...
            <div className="space-y-4">
              ...
              <div className="grid gap-3">
                <div className="space-y-1.5">
                  <Label htmlFor="m-name" className="text-xs text-muted-foreground">Nome completo *</Label>
                  <Input id="m-name" value={customer.name} onChange={(e) => setCustomer(c => ({ ...c, name: e.target.value }))} placeholder="Seu nome completo" className="h-11 bg-muted/30" />
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="m-email" className="text-xs text-muted-foreground">E-mail *</Label>
                  <Input id="m-email" type="email" value={customer.email} onChange={(e) => setCustomer(c => ({ ...c, email: e.target.value }))} placeholder="seu@email.com" className="h-11 bg-muted/30" />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <Label htmlFor="m-document" className="text-xs text-muted-foreground">CPF/CNPJ *</Label>
                    <Input id="m-document" value={customer.document} onChange={(e) => setCustomer(c => ({ ...c, document: formatDocument(e.target.value) }))} placeholder="000.000.000-00" maxLength={18} className="h-11 bg-muted/30" />
                  </div>
                  <div className="space-y-1.5">
                    <Label htmlFor="m-phone" className="text-xs text-muted-foreground">Telefone</Label>
                    <Input id="m-phone" value={customer.phone} onChange={(e) => setCustomer(c => ({ ...c, phone: formatPhone(e.target.value) }))} placeholder="(00) 00000-0000" maxLength={15} className="h-11 bg-muted/30" />
                  </div>
                </div>
              </div>
            </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <Input id="m-zipCode" value={address.zipCode}
                        onChange={(e) => {
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input id="m-state" value={address.state} onChange={(e) => setAddress(a => ({ ...a, state: e.target.value.toUpperCase() }))} placeholder="UF" maxLength={2} className="h-11 bg-muted/30" />
                  </div>
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="m-street" className="text-xs text-muted-foreground">Rua *</Label>
                  <Input id="m-street" value={address.street} onChange={(e) => setAddress(a => ({ ...a, street: e.target.value }))} placeholder="Nome da rua" className="h-11 bg-muted/30" />
                </div>
                <div className="grid grid-cols-3 gap-3">
                  <div className="space-y-1.5">
                    <Label htmlFor="m-number" className="text-xs text-muted-foreground">Número *</Label>
                    <Input id="m-number" value={address.number} onChange={(e) => setAddress(a => ({ ...a, number: e.target.value }))} placeholder="Nº" className="h-11 bg-muted/30" />
                  </div>
                  <div className="space-y-1.5">
                    <Label htmlFor="m-complement" className="text-xs text-muted-foreground">Complemento</Label>
                    <Input id="m-complement" value={address.complement} onChange={(e) => setAddress(a => ({ ...a, complement: e.target.value }))} placeholder="Apto, sala..." className="h-11 bg-muted/30" />
                  </div>
                  <div className="space-y-1.5">
                    <Label htmlFor="m-neighborhood" className="text-xs text-muted-foreground">Bairro *</Label>
                    <Input id="m-neighborhood" value={address.neighborhood} onChange={(e) => setAddress(a => ({ ...a, neighborhood: e.target.value }))} placeholder="Bairro" className="h-11 bg-muted/30" />
                  </div>
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="m-city" className="text-xs text-muted-foreground">Cidade *</Label>
                  <Input id="m-city" value={address.city} onChange={(e) => setAddress(a => ({ ...a, city: e.target.value }))} placeholder="Cidade" className="h-11 bg-muted/30" />
                </div>
              </div>
            </div>
            ...
            {paymentMethod === 'pix' && (
              <div className="flex items-center gap-3 p-4 bg-primary/5 rounded-xl border border-primary/20">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">QR Code gerado pelo app do seu banco</p>
                </div>
              </div>
            )}
            ...
            {paymentMethod === 'credit_card' && (
              <div className="space-y-4">
                <div className="space-y-1.5">
                  <Label htmlFor="m-cardNumber" className="text-xs text-muted-foreground">Número do cartão</Label>
                  <Input id="m-cardNumber" value={card.number} onChange={(e) => setCard(c => ({ ...c, number: formatCardNumber(e.target.value) }))} placeholder="0000 0000 0000 0000" maxLength={19} className="h-11 bg-muted/30" />
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="m-holderName" className="text-xs text-muted-foreground">Nome no cartão</Label>
                  <Input id="m-holderName" value={card.holderName} onChange={(e) => setCard(c => ({ ...c, holderName: e.target.value.toUpperCase() }))} placeholder="NOME COMO ESTÁ NO CARTÃO" className="h-11 bg-muted/30" />
                </div>
                <div className="grid grid-cols-3 gap-3">
                  <div className="space-y-1.5">
                    <Label htmlFor="m-expMonth" className="text-xs text-muted-foreground">Mês</Label>
                    <Input id="m-expMonth" value={card.expMonth} onChange={(e) => setCard(c => ({ ...c, expMonth: e.target.value.replace(/\D/g, '') }))} placeholder="MM" maxLength={2} className="h-11 text-center bg-muted/30" />
                  </div>
                  <div className="space-y-1.5">
                    <Label htmlFor="m-expYear" className="text-xs text-muted-foreground">Ano</Label>
                    <Input id="m-expYear" value={card.expYear} onChange={(e) => setCard(c => ({ ...c, expYear: e.target.value.replace(/\D/g, '') }))} placeholder="AA" maxLength={2} className="h-11 text-center bg-muted/30" />
                  </div>
                  <div className="space-y-1.5">
                    <Label htmlFor="m-cvv" className="text-xs text-muted-foreground">CVV</Label>
            ...
            {paymentMethod === 'boleto' && (
              <div className="flex items-center gap-3 p-4 bg-muted/50 rounded-xl border border-dashed">
                <div className="h-10 w-10 rounded-full bg-muted flex items-center justify-center shrink-0"><Clock className="h-5 w-5 text-muted-foreground" /></div>
                <div>
          ...
          <div className="p-4 md:p-6 border-t bg-background/80 backdrop-blur-sm space-y-4">
            <Button onClick={handleSubmit} className="w-full h-12 text-base gap-2 shadow-lg" disabled={loading}>
              {loading ? (
                <><Loader2 className="h-4 w-4 animate-spin" />Processando...</>
              ) : (
                <><Lock className="h-4 w-4" />Pagar {formatPriceBRL(course.price)}</>
              )}
            </Button>
            <div className="flex items-center justify-center gap-2 text-xs text-muted-foreground">
              <ShieldCheck className="h-4 w-4 text-success" /><span>Pagamento 100% seguro via Pagar.me</span>
            </div>
          </div>
        </div>
      )}
    ...
    return (
      <Drawer open={open} onOpenChange={(o) => { if (!o) resetModal(); onOpenChange(o); }}>
        <DrawerContent className="max-h-[95vh]">
          <DrawerHeader className="sr-only"><DrawerTitle>Finalizar Compra</DrawerTitle></DrawerHeader>
          <CheckoutContent />
        </DrawerContent>
      </Drawer>
    );
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) resetModal(); onOpenChange(o); }}>
      <DialogContent className="max-w-md p-0 gap-0 overflow-hidden max-h-[90vh]">
        <DialogHeader className="sr-only"><DialogTitle>Finalizar Compra</DialogTitle></DialogHeader>
        <CheckoutContent />
      </DialogContent>
    </Dialog>
  );
